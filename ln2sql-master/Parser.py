        ...
        for table_of_from in self.tables_of_from:
            ...
            for i in range(0, len(column_offset)):
                ...
                if len(self.columns_of_values_of_where) > i:
                    ##SHAKAR BHATTARAI COMMMENT IMPORTANT
...
class Parser:
    ...
    def get_prefix_values(self):
        return self.prefix_values,self.columns_where
        ...
        if irext:
            ...
            for idx,assigner in enumerate(assignment_list):
                ...
                if assigner in self.like_keywords:
                    ...
                    if assigner.strip()=='in':
                        irext = irext.replace(assigner, str(" " + "shakarassigner" + " "))
                    elif str.lower(assigner.strip())=="between":
                        irext = irext.replace(assigner, str(" " + "$H@K@r" + " "))
                    else:
            ...
            for idx, x in enumerate(irext_list):
                ...
                if( x == 'shakarassigner')and index+1< len(irext_list) :
                    #Shakar Bhattarai super important
                    cols=[]

                    while not (irext_list[index].__contains__(')')):
                        cols.append(irext_list[index])
                        index=index+1
                        if index==len(irext_list)-1:
                            break
                    cols.append(irext_list[index])

                    columns_of_values_of_where.append(','.join(cols))
                if (x == '$H@K@r') and index + 1 < len(irext_list):
                    cols=[]
                    while not (irext_list[index].__contains__(')')):
                        cols.append(irext_list[index])
                        index = index + 1
                ...
                if x == maverickjoy_general_assigner:
                    # SHAKAR BHATTARAI VERY IMPORTANT ACTUAL
                ...
                if x == '$H@K@r':
                    ...
                    print irext_list[index]
                    columns_of_values_of_where.append(str("IN '" + str(irext_list[index]).replace('<_>', ' ') + "'"))
                ...
                for column in columns:
                    ...
                    if (words[i] == column.get_name()) or (words[i] in column.get_equivalences()):
                        ...
                        if number_of_table == 0:
                            ...
                            self.prefix_values.append(words[i-1])
                            self.columns_where.append(column.get_name())
